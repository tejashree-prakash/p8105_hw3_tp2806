...
accel_df <- 
  read_csv("problem3_data/nhanes_accel.csv") %>%
  janitor::clean_names() 
knitr::kable(head(accel_df, 5))

covar_df <- 
  read_csv("problem3_data/nhanes_covar.csv", skip = 4) %>% #skip the first few rows that's data dictionary info 
  janitor::clean_names()
knitr::kable(head(covar_df, 5))
